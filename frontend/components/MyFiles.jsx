  ...
  return (
    <div className="w-[85vw] h-[82.5vh]   flex bg-[#F9F9F9] ">
      <div className="right  w-full bg-[#FAFAFA] p-4">
        <h1 className="text-[10px] text-[#898a89] tracking-[1.6px] font-semibold  mb-2">
          {Array.isArray(data?.path)
            ? data.path.map((word) => word.toUpperCase()).join(" / ")
            : ""}
        </h1>
          {folders.length === 0 ? (
          <div></div>
        ) : (
          <div className="folder  w-full h-[24vh] flex gap-5 text-[#474747] select-none ">
            <Swiper
        ...
        <h1 className="text-[12px] text-[#898a89] tracking-[1.6px] font-semibold  mb-2">Files</h1>
        <div
          className={`file w-full ${screen ? "h-[59vh]" : "h-[83vh]"} bg-[#FFFFFF] border border-[#c6c6c6] rounded-[7px]  overflow-hidden `}
        >
          ...
          {files.length == 0 ? (
            <div
              className={`w-full h-full text-[2vw] text-[#7e7e7e] flex items-center flex-col justify-center gap-3 pb-15`}
            >
              <img
                    src="/empty-box.png"
                  ...
                  <h2 className="text-lg font-semibold">You don't have any files yet</h2>

                  <h2 className="text-lg font-semibold -mt-2.25">upload your file to get started</h2>
              <label
                className="w-38 h-12  flex items-center text-xl justify-center gap-3  bg-blue-500 text-white text-[18px] rounded-lg cursor-pointer"
                htmlFor="uploadinput"
              >
                <LuUpload />
                upload
